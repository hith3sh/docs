...
<AccordionGroup>
  ...
  <Accordion title="ClawLink tools are missing in OpenClaw">
    ...
    - Restart the client (for OpenClaw, `openclaw gateway restart`) so the plugin reloads.
    - Confirm the device is paired and your apps show as connected in **Dashboard > Connections**.
...
For narrower client-side issues (OpenClaw gateway behavior, session routing, token usage), see the official OpenClaw documentation. For anything between your agent and ClawLink — connections, pairing, execution failures — start in **Usage** and work through the common fixes above.
